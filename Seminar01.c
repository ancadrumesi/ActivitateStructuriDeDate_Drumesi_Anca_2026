...
struct Student {
	...
	char* nume;
	float medie;
	char initiala; //1 char ocupa 1B, 1 octet, 8biti
};
...
struct Student initializare(int id, int grupa, char* nume, float medie, char initiala) {
	...
	s.id = id;
	s.grupa = grupa;
	s.nume = (char*)malloc(sizeof(char) * (strlen(nume) + 1)); //alocam spatiu pe heap
	strcpy(s.nume, nume);
	s.medie = medie;
	s.initiala = initiala;
	return s;
...
void modifica_Nume(struct Student* s, char* noulNume) {
	...
	//-> e un shortcut pentru (*s)
	//modificarea unui atribut=SETTER si folosim *s
}
...
char* comapara_Studenti(struct Student student1, struct Student student2) {
	//nu facem nicio modificare deci nu punem *student1
...
int main() {
	...
	printf("%s", comapara_Studenti(s, s2));
	dezalocare(&s);
